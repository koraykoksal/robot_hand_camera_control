...
OE_MIN_CUTOFF = 2.5              # küçült -> daha durağan/yumuşak, büyüt -> daha çevik
OE_BETA = 1.0                    # büyüt -> hızlı harekette daha az gecikme
...
# =========================================================================
# 8) OPENCV EL TAKİBİ (v2)  — MediaPipe YOK
# =========================================================================
# Yöntem: ten rengi maskesi -> en büyük kontur (el) -> avuç merkezi/yarıçapı
# (distance transform) -> her parmak için AÇISAL SEKTÖR içindeki en uzak
# kontur mesafesi -> parmak uzunluğu -> kıvrım.
#
# ÇALIŞMA KOŞULLARI (önemli):
#  - Elini ROI kutusunun içinde tut, parmaklar YUKARI baksın, avuç kameraya dönük.
#  - Arka plan sade olsun (ten rengine yakın duvar/ahşap sorun çıkarır).
#  - Yüz/kol ROI'ye girmesin (ROI bu yüzden var).

# İlgi alanı (ROI): kare içinde elin duracağı kutu. Oranlar (0..1): x, y, genişlik, yükseklik
ROI_REL = (0.55, 0.10, 0.42, 0.80)

# Ten rengi eşikleri (YCrCb). Kalibrasyon ('k') ile otomatik güncellenir.
SKIN_CR = (133, 180)
SKIN_CB = (77, 130)
# 'k' ile örnekleme yapıldığında eşik genişliği (std katı)
SKIN_LEARN_TOL = 2.5

# Gürültü temizleme
MORPH_KERNEL = 5
BLUR_KERNEL = 5
MIN_CONTOUR_AREA = 3000          # bundan küçük konturu el sayma

# Parmak sektörü yarı-genişliği (derece). Parmaklar birbirine karışıyorsa küçült.
SECTOR_HALF_DEG = 13.0

# Parmak uzunluğu normalizasyonu (avuç yarıçapının katı olarak):
# uzunluk <= FINGER_MIN_R -> tam kapalı(1),  >= FINGER_MAX_R -> tam açık(0)
FINGER_MIN_R = 1.35
FINGER_MAX_R = 2.60

# ROI ve maskeyi ekranda göster (ayarlarken çok işe yarar)
SHOW_MASK = True
CV_CALIB_PATH = os.path.join(_APP_DIR, "models", "cv_calibration.json")

# --- EL DIŞINDAKİ NESNELERİ ELEME (v2) ---
# Kontur bu testleri geçmezse EL SAYILMAZ (yüz, kol, mobilya, duvar lekesi vb.)
MAX_AREA_RATIO = 0.55            # ROI'nin bu oranından büyük leke = arka plan, ele
SOLIDITY_RANGE = (0.35, 0.93)    # el; yüz gibi dolu/yuvarlak lekeler >0.93 olur ve elenir
ASPECT_RANGE = (0.35, 2.60)      # en/boy; aşırı ince şerit (kol, kablo) elenir
